...
void trivium_init(const void *key, uint16_t keysize_b,
    ...
    if(is_debug_mode) {
        std::cout << "[DEBUG trivium_init] Processing key bytes" << std::endl;
    }
    ...
    do {
        ...
        if(is_debug_mode) {
            std::cout << "[DEBUG trivium_init] Key byte " << (int)c1 
    ...
    if(is_debug_mode) {
        std::cout << "[DEBUG trivium_init] Processing IV bytes" << std::endl;
    }
    ...
    do {
        ...
        if(is_debug_mode) {
            std::cout << "[DEBUG trivium_init] IV byte " << (int)c1 
                      << ": original=" << std::hex << (int)t1 
                      << ", reversed=" << (int)t2 << std::dec << std::endl;
        }
    } while(c1 != 0);
...
std::bitset<MAX_SIZE> stream_cipher_TRIVIUM(std::bitset<MAX_SIZE> input, int n, int /*m*/) {
    ...
    // Configuración de clave e IV (80 bits cada uno como en el ejemplo)
    uint8_t key[10] = {
    ...
    if(is_debug_mode) {
        std::cout << "[DEBUG stream_cipher_TRIVIUM] Using default key and IV" << std::endl;
    }
    ...
    for (size_t i = 0; i < bits_to_process; i += 8) {
        ...
        for (size_t j = 0; j < bits_in_this_byte; ++j) {
            ...
            if(is_debug_mode && j < 4) { // Limit debug output for first few bits
